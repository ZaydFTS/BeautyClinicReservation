...
export function AdminFinancialsPage() {
 ...
 <div className="font-bold text-secondary">{fmt(v.completed)}</div>
 {v.pending > 0 && (
 <div className="text-xs text-primary">{fmt(v.pending)} pending</div>
 )}
 </div>
 </div>
 ))}
 ...
 : t.status ==="CANCELLED"
 ?"bg-blush text-secondary border-outline-variant"
 :"bg-blush-strong text-secondary border-outline-variant"
 }
 >
 {t.status}
 </Badge>
