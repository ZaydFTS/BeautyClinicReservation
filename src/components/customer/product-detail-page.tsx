...
export function ProductDetailPage({ route }: { route: Extract<Route, { name:"product_detail" }> }) {
 ...
 {/* Image — premium panel */}
 <div className="relative aspect-square overflow-hidden rounded-3xl border border-outline-variant/70 bg-blush">
 {/* Decorative orbs (visible around image edges) */}
 <div
 className="pointer-events-none absolute -right-12 -top-12 h-48 w-48 rounded-full bg-primary/15"
 aria-hidden
 />
 <div
 className="pointer-events-none absolute -bottom-12 -left-12 h-40 w-40 rounded-full bg-primary/15"
 aria-hidden
 />
 {product.imageUrl ? (
 <img
 ...
 <Leaf className="h-24 w-24 text-primary-container/70" />
 </div>
 )}
 ...
 <div className="absolute start-5 top-5 inline-flex items-center gap-1.5 rounded-full border border-outline-variant/60 bg-white/70 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-secondary shadow-md backdrop-blur-md">
 ...
 {product.category && (
 <div className="mb-3 text-xs font-medium uppercase tracking-wider text-secondary/70">
 ...
 <span className="text-primary">{formatMoney(product.price)}</span>
 </div>
 ...
 <Card className="mt-6 border-outline-variant/70 bg-blush">
 <CardContent className="p-4 sm:p-5">
 <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
 {/* Quantity selector */}
 <div className="inline-flex h-11 shrink-0 items-center gap-1 rounded-full border border-outline-variant/70 bg-white/80 p-1 shadow-sm backdrop-blur-sm">
 <Button
 variant="ghost"
 size="icon"
 className="h-9 w-9 rounded-full text-secondary hover:bg-blush hover:text-secondary"
 ...
 <Button
 size="lg"
 className="btn-shimmer flex-1 gap-2 rounded-full bg-primary"
 onClick={handleAdd}
 ...
 <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-blush/70">
