...
export function BookingPage({ route }: { route: Extract<Route, { name:"booking" }> }) {
 ...
 {/* Slots */}
 {!selectedService ? (
 <div className="mt-6 relative overflow-hidden rounded-2xl border border-dashed border-outline-variant/70 bg-blush">
 ...
 : isAvailable
 ?"border-border hover:border-outline hover:bg-blush hover:scale-[1.02]"
 ...
 {t("booking.fullName")} <span className="text-primary">{t("booking.required")}</span>
 </Label>
 <Input
 id="name"
 ...
 {t("booking.phone")} <span className="text-primary">{t("booking.required")}</span>
 </Label>
 <Input
 id="phone"
 value={form.phone}
